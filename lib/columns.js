...
import Diagonals from '../lib/diagonal.js';
import $ from 'jquery';
  ...
  fillCirclesFromBottom(circles) {
    for (let i = circles.length - 1; i >= 0; i--) {
      if (this.isCircleEmpty(circles[i]) && this.circleTracker % 2 === 0) {
        ...
        this.circleTracker++;
        Row.pieces($(circles[i]).attr('row-id'), 'black', $(circles[i]).attr('column-id'));
        Diagonals.createBoard($(circles[i]).attr('row-id'), 'black', $(circles[i]).attr('column-id'));
        return null;
      } else {
        if (this.isCircleEmpty(circles[i]) && this.circleTracker % 2 !== 0) {
          $(circles[i]).attr('class', 'redPiece');
          this.circleTracker++;
          Row.pieces($(circles[i]).attr('row-id'), 'red', $(circles[i]).attr('column-id'));
          Diagonals.createBoard($(circles[i]).attr('row-id'), 'red', $(circles[i]).attr('column-id'));
          return null;
        }
      }
    }
